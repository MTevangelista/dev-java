...
public final class Main {
    ...
    private static void start(int option, ArrayList<Account> accounts, Connection conn) {
        switch (option) {
            case 1:
                registerAccount(accounts, conn);
                break;
            case 2:
                editAccount(accounts, conn);
                break;
            case 3:
                removeAccount(accounts, conn);
                break;
            case 4:
                showReportOptions(accounts, conn);
                break;
            default:
    ...
    private static void registerAccount(ArrayList<Account> accounts, Connection conn) {
        int accountType; 
        int accountNumber;
        ...
        switch (accountType) {
            case 1:
                ...
                break;
            case 2: 
                setAccountPJData(accounts, accountNumber, conn);
                break;
            default:
    ...
    private static void setAccountPJData(ArrayList<Account> accounts, int accountNumber, Connection conn) {
        ...
        DBManager.addAccountPJ(conn, accountPJ);
        accounts.add(accountPJ);
        Util.showMessage(MessageTypesEnum.OUT, Constants.ACCOUNT_REGISTER_SUCCESSFULLY);
    }
    
    private static void editAccount(ArrayList<Account> accounts, Connection conn) {
        int accountNumber;
...
//        account = Util.getAccount(accountNumber, accounts);
        ...
        switch (operationType) {
            case CREDIT:
                ...
                DBManager.updateAccountPJ(conn, accountNumber, operationValue, Constants.CREDIT);
            ...
            case DEBIT:
                operationValue = Util.readDoubleValue(Constants.ENTER_YOUR_OPERATION_VALUE);
                DBManager.updateAccountPJ(conn, accountNumber, operationValue, Constants.DEBIT);
                //account.setAccountBalance(account.getAccountBalance() - operationValue);
...
//                }
    ...
    private static void removeAccount(ArrayList<Account> accounts, Connection conn) {
        int accountNumber;
//        int accountIndex;
...
//        }
        ...
        accountNumber = Util.readIntValue(Constants.ENTER_YOUR_ACCOUNT_NUMBER);
        DBManager.removeAccountPJ(conn, accountNumber);
        //accounts.remove(accountNumber);
        Util.showMessage(MessageTypesEnum.OUT, Constants.ACCOUNT_REMOVED_SUCCESSFULLY);
    }
    
    private static void showReportOptions(ArrayList<Account> accounts, Connection conn) {
        int option;
        String response;
...
//        }
        ...
        switch (option) {
            ...
            case 2: 
                ...
                response = DBManager.getAllAccounts(conn);
                Util.showMessage(MessageTypesEnum.OUT, response);
                break;
            case 3:
                getAccountsByType(accounts);
                break;
            case 4:
                getAccountOperation(accounts, conn);
                break;
        }
    }
    ...
    private static void getAccountOperation(ArrayList<Account> accounts, Connection conn) {
        int accountNumber;
        String response;
        
        accountNumber = Util.readIntValue(Constants.ENTER_YOUR_ACCOUNT_NUMBER);
        response = DBManager.getAllOperations(conn, accountNumber);
        Util.showMessage(MessageTypesEnum.OUT, response);
//        if (!DataValidation.hasRepeatedAccount(accountNumber, accounts)) {
//            Util.showMessage(MessageTypesEnum.ERR, Constants.ACCOUNT_NOT_FOUND);
//            return;
